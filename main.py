...
print('Barycenter (brute-force)', m)
marginals = (m + 1) / 2
...
def marginals_factorgraph(W, y):
    ...
    def compute_message_fv2v(idx, v):
        if (('fv2v', idx), idx, v) not in messages:
            messages[(('fv2v', idx), idx, v)] = torch.exp(y[idx] * v)
            # print('NEW', end='\t')
        # print(
    ...
    def compute_message_fh2h(idx, h):
        if (('fh2h', idx), idx, h) not in messages:
            messages[(('fh2h', idx), idx, h)] = torch.exp(y[n_v + idx] * h)
            # print('NEW', end='\t')
        # print(
    ...
    def compute_message_f2v(src, dst, v):
        if ('f2v', src, dst, v) not in messages:
            i, j = src
            messages[('f2v', src, dst, v)] = \
                torch.exp(W[i, j] * v * 1) * \
                compute_message_h2f(j, src, 1) \
                + torch.exp(W[i, j] * v * (-1)) * \
                compute_message_h2f(j, src, -1)
            # print('NEW', end='\t')
    ...
    def compute_message_f2h(src, dst, h):
        if ('f2h', src, dst, h) not in messages:
            i, j = src
            messages[('f2h', src, dst, h)] = \
                torch.exp(W[i, j] * h * 1) * \
                compute_message_v2f(i, src, 1) \
                + torch.exp(W[i, j] * h * (-1)) * \
                compute_message_v2f(i, src, -1)
            # print('NEW', end='\t')
    ...
    m = torch.zeros(2, n)
    for i in range(n_v):
        m[0, i] = compute_marginal_v(i, 1)
        m[1, i] = compute_marginal_v(i, -1)

    for j in range(n_v, n):
        m[0, j] = compute_marginal_h(j - n_v, 1)
        m[1, j] = compute_marginal_h(j - n_v, -1)

    m = m / m.sum(0, keepdim=True)
    m = m[0] - m[1]
    print(m)


marginals_factorgraph(W, y)
