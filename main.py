...
def marginals_factorgraph(W, y):
    ...
    def compute_message_fv2v(i_v, v):
        ...
        if ('fv2v', i_v, v) not in messages:
            if DEBUG:
                print(f'[COMPUTE] {_id}')

            messages[('fv2v', i_v, v)] = y[:, [i_v]] * v
        else:
            if DEBUG:
                print(f'[CACHE] {_id}')
    ...
    def compute_message_fh2h(i_h, h):
        ...
        if ('fh2h', i_h, h) not in messages:
            if DEBUG:
                print(f'[COMPUTE] {_id}')

            messages[('fh2h', i_h, h)] = y[:, [n_v + i_h]] * h
        else:
            if DEBUG:
                print(f'[CACHE] {_id}')
    ...
    def compute_message_f2v(i_v, i_h, v):
        ...
        if ('f2v', i_v, i_h, v) not in messages:
            if DEBUG:
                print(f'[COMPUTE] {_id}')
    ...
    def compute_message_f2h(i_v, i_h, h):
        ...
        if ('f2h', i_v, i_h, h) not in messages:
            if DEBUG:
                print(f'[COMPUTE] {_id}')
    ...
    def compute_message_h2f(i_v, i_h, h):
        ...
        if ('h2f', i_v, i_h, h) not in messages:
            if DEBUG:
                print(f'[COMPUTE] {_id}')
    ...
    def compute_message_v2f(i_v, i_h, v):
        ...
        if ('v2f', i_v, i_h, v) not in messages:
            if DEBUG:
                print(f'[COMPUTE] {_id}')
