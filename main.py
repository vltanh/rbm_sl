...
from energy_based import energy_rbm, barycenter, distribution, partition
...
def marginals_factorgraph(W, y):
    ...
    for i in range(n_v):
        m[:, 0, i] = compute_logmarginal_v(i, 1).reshape(-1)
        m[:, 1, i] = compute_logmarginal_v(i, -1).reshape(-1)

    for j in range(n_v, n):
        m[:, 0, j] = compute_logmarginal_h(j - n_v, 1).reshape(-1)
        m[:, 1, j] = compute_logmarginal_h(j - n_v, -1).reshape(-1)
